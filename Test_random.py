...
n_comp = (np.array([i for i in xrange(10)])+1).reshape(-1)
...
for i in tqdm(xrange(n_comp.shape[0])):
    ...
    P = dict_learning(Sigma, n_components = n, alpha = x, max_iter=100, tol=1e-08)
    if(P[1].shape[0] == n):
        Trans = P[1]
    else:
...
P = np.argsort(np.array(n_comp))
P = P.astype(int)
print len(Results), P
Res = np.array(Results).reshape(-1)
plt.plot(n_comp[P], Res[P])
plt.ylabel('Accuracy')
